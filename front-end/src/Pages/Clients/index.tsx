...
const { user } = useUser<AdminI>();
	const { data: response, isError, isLoading, isFetching, refetch } = useGetAdminClientsQuery(user._id); */
	const { data: response, isError, isLoading, isFetching, refetch } = useGetClientsQuery();

	const data = (response?.data || []) as ClientI[];
	const { Notify, Errofy } = useNotification();
	const dynamicFilter = useDynamicFilter<ClientI>(data, {
		enabled: {
			name: "Status",
			enums: [
		...
		<UserTable<ClientI>
			...
			labels={{
				...
				upsIntegrationSet: {
					name: "Integration",
					render: (l) => (
						<div className={`badge text-ellipsis overflow-hidden whitespace-nowrap ${l ? "badge-accent" : "badge-warning"}`}>
