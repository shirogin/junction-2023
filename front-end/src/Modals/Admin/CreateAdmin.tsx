...
import { useCheckAvailableAdminMutation, useCreateAdminMutation } from "@/app/backend/export/admin";
...
export default function CreateAdmin({ closeModal }: { closeModal: () => void }) {
	const [CreateAnAdmin, { isLoading }] = useCreateAdminMutation();
	const { Notify, Errofy } = useNotification();
	const { currentDesk } = useStoreDesk();
	const [CheckAvailable] = useCheckAvailableAdminMutation();
			...
			CreateAnAdmin({
				...
				.then((response) => {
					...
					if (user && user.kind === "Admin") {
						Notify("Creating an admin", response.message);
						closeModal();
					} else {
						Errofy("Creating an admin", { message: "Something went wrong" });
					}
				})
				.catch((error) => {
					Errofy("Creating an admin", error);
				});
		},
	});
